...
@androidx.room.Database(entities = {PropertyInformation.class, RealEstateAgent.class, PointOfInterest.class, PropertyMedia.class, PropertyLocation.class}, version = 2, exportSchema = false)
public abstract class RealEstateManagerRoomDatabase extends RoomDatabase {
    ...
    //do before testing content provider on new device in order to insert mock data
    @SuppressWarnings("unused")
    private static Callback prepopulateDatabaseForTest() {
        return new Callback() {
            @Override
            public void onCreate(@NonNull SupportSQLiteDatabase db) {
                super.onCreate(db);
